...
<template>
    <article class="media">
        ...
        <div :class="`modal ${editModalActive ? 'is-active' : ''}`">
            ...
            <button class="modal-close is-large" area-label="close" @clock="toggleEditModal"></button>
        </div>
    </article>
</template>
<script>
export default {
    ...
    computed: {
        isQuestionUser() {
            ...
            return this.$store.getters.user.id === this.question.user.id ? true : false;
        }
    },
    methods: {
        toggleRemoveModal() {
        ...
        removeQuestion(id) {
            // 削除処理、removeQuestion処理にidを取り、actionにdispatchする
